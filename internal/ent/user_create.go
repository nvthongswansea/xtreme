...
func (uc *UserCreate) AddDirectoryIDs(ids ...string) *UserCreate {
	uc.mutation.AddDirectoryIDs(ids...)
	return uc
}

// AddDirectories adds the "directories" edges to the Directory entity.
func (uc *UserCreate) AddDirectories(d ...*Directory) *UserCreate {
	ids := make([]string, len(d))
	for i := range d {
		ids[i] = d[i].ID
	}
	return uc.AddDirectoryIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.DirectoriesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
