...
// HasDirectories applies the HasEdge predicate on the "directories" edge.
func HasDirectories() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(DirectoriesTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, DirectoriesTable, DirectoriesColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasDirectoriesWith applies the HasEdge predicate on the "directories" edge with a given conditions (other predicates).
func HasDirectoriesWith(preds ...predicate.Directory) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(DirectoriesInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, DirectoriesTable, DirectoriesColumn),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
