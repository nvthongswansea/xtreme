...
func (fq *FileQuery) QueryParent() *DirectoryQuery {
	query := &DirectoryQuery{config: fq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := fq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (fq *FileQuery) WithParent(opts ...func(*DirectoryQuery)) *FileQuery {
	query := &DirectoryQuery{config: fq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (fq *FileQuery) sqlAll(ctx context.Context) ([]*File, error) {
	...
	if fq.withOwner != nil || fq.withParent != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := fq.withParent; query != nil {
		...
		nodeids := make(map[string][]*File)
		for i := range nodes {
			if nodes[i].directory_child_files == nil {
				continue
			}
			fk := *nodes[i].directory_child_files
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "directory_child_files" returned %v`, n.ID)
			}
			for i := range nodes {
