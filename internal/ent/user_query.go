...
type UserQuery struct {
	...
	// eager-loading edges.
	withFiles       *FileQuery
	withDirectories *DirectoryQuery
...
// QueryDirectories chains the current query on the "directories" edge.
func (uq *UserQuery) QueryDirectories() *DirectoryQuery {
	query := &DirectoryQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withDirectories: uq.withDirectories.Clone(),
...
// WithDirectories tells the query-builder to eager-load the nodes that are connected to
// the "directories" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithDirectories(opts ...func(*DirectoryQuery)) *UserQuery {
	query := &DirectoryQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withDirectories = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withDirectories; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[string]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Directories = []*Directory{}
		}
		query.withFKs = true
		query.Where(predicate.Directory(func(s *sql.Selector) {
			s.Where(sql.InValues(user.DirectoriesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_directories
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_directories" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_directories" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Directories = append(node.Edges.Directories, n)
		}
	}
